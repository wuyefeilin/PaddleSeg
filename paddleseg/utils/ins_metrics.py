...
class AveragePrecision(object):
    ...
    def __init__(self, num_classes, overlaps=0.5):
        super().__init__()
    ...
    def compute(self, preds, gts, ignore_mask=None):
        """
...
def convert_pred_map(seg_pred, ins_map):
    """
    Convet the ground truth with format (h*w) to the format that satisfies the AP calculation.

    Args:
        seg_pred (np.ndarray): the sementic segmentation map with shape C * H * W. Value is probability.
        ins_map (np.ndarray): the instance segmentation map with shape H * W. Value is 1001, 1002, ..., 2001, 2002, ...,
            e.x. 1001 means label 1 and instance 1.
